...
- **Confirmed Travel Information layout:** Use the same floating brown return arrow after the section buttons scroll above the header, returning to the first section button on desktop and mobile. Pair “Visa-Free Entry” on the left with inset Local Knowledge on the right, using the homepage 56rem left column and 1400px stacking breakpoint, without a duplicate Local Knowledge section. The “Ready when you are” panel spans the full content width. Show the four visa cost cards in one desktop row, reflowing on smaller screens; prices use bold body-sized text.
