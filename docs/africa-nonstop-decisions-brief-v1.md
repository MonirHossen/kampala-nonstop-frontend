...
- **Confirmed desktop navigation:** **Home, Hot Picks, Guide, Discover, Services, Concierge, Support, Dashboard**, followed by heart/favourites, notification bell and **Plan My Trip**.
- **Confirmed Services/Concierge placeholders (September 2026):** Add `/services` and `/concierge` immediately after Discover in desktop and mobile navigation, preserving existing account and waitlist actions while the full navigation is still being built. Use plain text introductions matching Discover instead of image heroes, retaining the existing page titles and supporting text. Below each body subheading, use the approved Services/Concierge trip-planning copy from chat; catalogue, requests, bookings and backend integration remain outside this change.
- **Confirmed Discover placeholder (September 2026):** Add `/discover` between Guide and Services in both menus, per the explicit chat instruction overriding the handover order. Use a text introduction headed “Your adventure awaits” in place of the Guide hero, followed by the approved “Discover Uganda your way.” trip-planning copy; remove the discovery coming-soon message. Show ten editorial discovery themes, five listing-type links. Centre the introduction heading and its bold opening line; put the Bookmark copy in a separate left-aligned paragraph aligned with the page content. The landing page has no location control (location remains in the catalogue). These themes are not the database taxonomy. Catalogue navigation and URL filter state are now approved; backend catalogue integration remains out of scope.
- **Confirmed Discover theme tiles (September 2026):** Use the matching unnumbered discovery images from `public/img/originals` behind dark overlays, white category icons and labels. Use four columns on desktop and two on mobile, with `object-fit: cover` for undistorted cropping. Align category icons left, beside the label and vertically centred in the tile. Wrap Music, Nightlife & Entertainment over two lines: “Music, Nightlife &” then “Entertainment”. Centre tile labels at the introduction body size (1rem), regular weight, with a visible bottom-right arrow. Keep the ten current editorial theme labels, including Wellness & Relaxation mapped to the wellness-and-beauty image.
- **Confirmed Discover browse-by-type styling (September 2026):** Use rounded pale tiles with left-aligned, vertically centred dark green outline category icons (star for Experiences), centred 1rem regular-weight labels and visible bottom-right arrows, matching the supplied mock-up. Include “Looking for something specific? Explore our bookable options”. These options now link to the catalogue with their type selected.
- **Confirmed catalogue shell (September 2026):** `/discover/catalogue` is reached from interest and listing-type tiles. Reuse the site navbar and breadcrumb, with a sticky, horizontally scrollable filter bar for the existing editorial interests, five UI listing types and Kampala context. Keep selections in URL query parameters, with removable chips and clear all. Show an honest catalogue-not-yet-available state; omit unsupported curation, price, booking/secondary filters and listing actions until data exists. No new backend fields, fabricated inventory or trip workflow.
