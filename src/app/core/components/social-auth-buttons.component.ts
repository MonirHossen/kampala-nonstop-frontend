...
export class SocialAuthButtonsComponent {
  ...
  protected signIn(provider: SocialProvider): void {
    if (this.busy()) {
      return;
    }
    ...
    try {
      this.social.startOAuth(provider, this.returnUrl() || '/dashboard');
    } catch (err: unknown) {
      this.error.set(err instanceof Error ? err.message : `Unable to start ${provider} sign-in.`);
